...
| 20 | Base oficial do Node, **não** imagem endurecida de terceiro | distroless ou imagem endurecida comercial | Avaliado após as correções: a varredura ficou limpa, então a troca resolveria zero achados atuais sem ganho medido. O healthcheck já foi convertido para forma exec num commit seguinte, então esse custo deixou de existir; permanece adaptar os estágios ao contrato da imagem, respeitar o usuário provido por ela e validar um runtime sem shell. Reavaliar se CVEs voltarem a ser recorrentes na base oficial |
